...
class GsmModem(SerialComms):
    ...
    def __init__(self, port, baudrate=115200, incomingCallCallbackFunc=None, smsReceivedCallbackFunc=None, smsStatusReportCallback=None,fatalErrorCallbackFunc=None,unhandledNotificationCallbackFunc=None ,requestDelivery=True, AT_CNMI="", *a, **kw):
        super(GsmModem, self).__init__(port, baudrate, notifyCallbackFunc=self._handleModemNotification, *a, **kw)
        self.incomingCallCallback = incomingCallCallbackFunc or self._placeholderCallback                   # incomingCallCallback(call)               class IncomingCall
        self.smsReceivedCallback = smsReceivedCallbackFunc or self._placeholderCallback                     # smsReceivedCallback(sms)                 class ReceivedSms
        self.smsStatusReportCallback = smsStatusReportCallback or self._placeholderCallback                 # smsStatusReportCallback(sms report)      class StatusReport
        self.unhandledNotificationCallback = unhandledNotificationCallbackFunc or self._placeholderCallback # unhandledNotificationCallbackFunc(lines) class [str]
    ...
    def airplaneMode(self):
        """ Turn the modem into airplane mode, disable the simcard
        """
        return self.write('AT+CFUN=0')[0]

    def simStatus(self):
        return self.write('AT+CPIN?')[0]
    ...
    def initialize(self, pin=None, waitingForModemToStartInSeconds=0):
        """
         Initializes the modem and SIM card
    ...
    def processStoredSms(self, unreadOnly=False):
        ...
        if self.smsReceivedCallback:
            ...
            for msgStatus in states:
                ...
                for sms in messages:
                    self.smsReceivedCallback(self,sms)
        else:
    ...
    def __threadedHandleModemNotification(self, lines):
        ...
        self.unhandledNotificationCallback(self,lines)
    ...
    def _handleIncomingCall(self, lines):
        ...
        if call == None:
            ...
            self.activeCalls[callId] = call
        self.incomingCallCallback(self,call)
        ...
        try:
            ...
            if cdsiMatch:
                ...
                if self._smsStatusReportEvent:
                    ...
                    try:
                        self.smsStatusReportCallback(self,report)
                    except Exception:
                        self.log.error('error in smsStatusReportCallback', exc_info=True)
        except Exception as e:
            try:
                report = StatusReport(self, -1, None, None, datetime.now(), None, "Can't get report")
                self.smsStatusReportCallback(self,report)
            except Exception:
                self.log.error('error in smsStatusReportCallback', exc_info=True)
