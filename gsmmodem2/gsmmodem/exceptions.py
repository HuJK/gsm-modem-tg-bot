...
class GsmModemException(Exception):
    ...
    __err_dict__ = json.loads(open( os.path.join(script_dir, "exception.json")  ).read())
...
class CmeError(CommandError):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) >= 2:
            ...
            if str(code) in cls.__err_dict__["CME"]:
                cls._description = cls.__err_dict__["CME"][str(code)]
            if code == 11:
...
class CmsError(CommandError):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) >= 2:
            code = args[1]
            if str(code) in cls.__err_dict__["CMS"]:
                cls._description = cls.__err_dict__["CMS"][str(code)]
            if code == 330:
